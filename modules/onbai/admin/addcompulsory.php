...
if ( $nv_Request->get_int( 'edit', 'post', 0 ) == 1 )
{
	...
	{	
		$query = mysql_query("UPDATE `" . NV_PREFIXLANG . "_" . $module_data . "_compulsory` SET `".$key."` = " . $db->dbescape( $data ) . " WHERE `id` =" . $id . "");
	}
	if ( $query ) 
	{
		Header( "Location: " . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name."&" . NV_OP_VARIABLE . "=compulsory"); die();
	}
	else
	{
